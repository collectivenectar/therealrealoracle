...
#special thanks to: 
#https://github.com/3ddelano/discord-ui-godot/blob/main/scripts/ExpandableTextEdit.gd
...
signal pass_on_textedit_height(height)
...
func _update_height(count):
	...
	var new_height = lines_to_show * (line_height + line_spacing * 1.45) + 50
	rect_min_size.y = new_height
	# collectivenectar : new line here to get parent/sib height to match expandingtextedit height.
	if new_height > 150:
		emit_signal("pass_on_textedit_height", new_height)
