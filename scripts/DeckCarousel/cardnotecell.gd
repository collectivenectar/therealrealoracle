...
func _on_Tween_tween_completed(object, key):
	if object == get_node("Panel") and key == ":rect_position:x" :
		tween.interpolate_property($Panel, "rect_min_size:y", 200, 300, 0.1, Tween.TRANS_LINEAR, Tween.EASE_OUT, 0.2)
		tween.interpolate_property(self, "rect_min_size:y", 200, 300, 0.1, Tween.TRANS_LINEAR, Tween.EASE_OUT, 0.2)
		tween.interpolate_property($VBoxContainer, "rect_min_size:y", 154, 250, 0.1, Tween.TRANS_LINEAR, Tween.EASE_OUT, 0.2)
		tween.start()
