...
#FINISH LINE 21 AND 24 instructions
...
func _carousel_dragged_pos(window_position):
	...
	for i in 5: 
		if -card_zone < get_child(i).rect_position.x and get_child(i).rect_position.x < rect_size.x:
			#Below I need to add code to access the get_child(i).method to change the text in both richtextboxes 
			#to the descriptionLarge and descriptionSmall values present for global.carousel_choice[fposmod stuff]
			get_child(i).texture = global.carousel_choice[fposmod(int(window_position / card_zone) + (i - 2), global.carousel_choice.size())]['texture']
			#Then here (below) I need to remove this code since I will just pull directly from user data to fill any
			#personal notes on unique cards when the card is selected.
